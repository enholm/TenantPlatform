...
public sealed class OrganizationElementService(
    ...
    ITenantAuthorizationService authorization)
{
    ...
    public async Task<List<OrganizationElement>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        var accountId = await RequireAccountAsync(cancellationToken);
        await using var db = await dbFactory.CreateDbContextAsync(cancellationToken);
        return await db.OrganizationElements.AsNoTracking().Where(x => x.AccountId == accountId)
    ...
    public async Task SaveAsync(Guid? id, AccountRegisterInput input, CancellationToken cancellationToken = default)
    {
        ...
        OrganizationElement entity;
        if (id.HasValue)
        {
            entity = await db.OrganizationElements.SingleOrDefaultAsync(
                x => x.AccountId == accountId && x.Id == id.Value, cancellationToken)
                ?? throw new InvalidOperationException("AccountRegisterNotFound");
        }
        else
        {
            entity = new OrganizationElement { Id = Guid.NewGuid(), AccountId = accountId };
            db.OrganizationElements.Add(entity);
        }
    ...
    public async Task DeleteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        await using var transaction = await db.Database.BeginTransactionAsync(cancellationToken);
        await LockAccountAsync(db, accountId, cancellationToken);
        var entity = await db.OrganizationElements.SingleOrDefaultAsync(
        ...
        Guid? id, Guid? parentId, CancellationToken cancellationToken)
    {
        ...
        var parents = await db.OrganizationElements.AsNoTracking().Where(x => x.AccountId == accountId)
